/**
 * orchestrator-api — bounded context service (ADR-0001)
 * Port: :3098
 *
...
bus.connect().then(() => {
  busConnected = true;
  console.log("[orchestrator-api] connected to Redis bus");
}).catch((err) => {
  console.warn("[orchestrator-api] Redis bus connection failed (non-fatal):", err?.message);
});
...
console.log("[orchestrator-api] listening on :");

process.on("SIGTERM", () => {
  console.log("[orchestrator-api] shutting down");
  server.stop();
  process.exit(0);
});
